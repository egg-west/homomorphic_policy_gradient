...
class DrQV2Agent:
    def __init__(self, obs_shape, action_shape, device, lr, feature_dim, hidden_dim,
                 ...
                 stddev_schedule, stddev_clip, n_samples):
        self.device = device
    ...
    def update_critic(self, obs, action, reward, discount, next_obs, step):
        ...
        Q1_var = Q1.reshape((Q1.shape[0]//self.n_samples, self.n_samples, -1)).var(dim=1).mean()
        Q2_var = Q1.reshape((Q1.shape[0]//self.n_samples, self.n_samples, -1)).var(dim=1).mean()
        metrics['Q1_var'] = Q1_var.item()
        metrics['Q2_var'] = Q2_var.item()

        critic_loss += Q1_var + Q2_var
    ...
    def update(self, replay_iter, step):
        ...
        #print(f"before tile: {action.shape=}, {reward.shape=}, {discount.shape=}")
        action = action.tile((self.n_samples, 1))
        reward = reward.tile((self.n_samples, 1))
        discount = discount.tile((self.n_samples, 1))
        #print(f"{action.shape=}, {reward.shape=}, {discount.shape=}")

        # augment
        #print(f"{obs.shape=}")
        obs_tot = self.aug(obs.float().tile((self.n_samples, 1, 1, 1)))
        #obs = self.aug(obs.float())
        next_obs_tot = self.aug(next_obs.float().tile((self.n_samples, 1, 1, 1)))
        #print(f"{obs_tot.shape=}")

        # encode
        obs = self.pixel_encoder(obs_tot)
        with torch.no_grad():
            next_obs = self.pixel_encoder(next_obs_tot)
        #print(f"after encoder: {obs.shape=}")
